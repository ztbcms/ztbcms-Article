...
class ArticleGroupService extends BaseService
{
    ...
    static function getGroupList($where,$order,$page,$limit){
        $res = self::select('commonly_article_group',$where,$order,$page,$limit);
    ...
    static function addEditGroup($data){
        $commonlyGroupModel = new CommonlyArticleGroupModel();
    ...
    static function getChildList($id, $where){
        $where['parent_id'] = $id;
        $where['is_delete'] = '0';
        $data = M('commonly_article_group')->field('id,listorder,title,is_display,parent_id,lv,cover_url')->where($where)->order('`listorder` DESC')->select() ?: [];
        $offset = 0;
    ...
    static function getCateList($type,$id,$current_id){
        ...
        $data = M('commonly_article_group')->field('id,title,lv')->where($where)->order('`listorder` DESC')->select() ?: [];
        if(!$id) array_unshift($data, ['id' => '0', 'title' => '顶级分类', 'lv' => 1]);
        foreach($data as &$v){
            if($v['id']){
                $has_child = M('commonly_article_group')->where(['parent_id' => $v['id'],'is_delete'=>'0'])->count();
    ...
    static function getPid($id, $need_self = false){
        ...
        $pid = M('commonly_article_group')->where(['id' => $id,'is_delete'=>'0'])->getField('parent_id');
        if($pid){
            array_unshift($return, $pid);
            $pid = M('commonly_article_group')->where(['id' => $pid,'is_delete'=>'0'])->getField('parent_id');
     ...
     * @return array
     */
    static function getGroupTreeByType($type, $pid, $lv = 1){
        $result = [];
        $cates = M('commonly_article_group')->where([
            'type' => $type,
            'parent_id' => $pid,
            'lv' => $lv,
            'is_delete' => 0,
            'is_display' => 1
        ])->order('lv asc, listorder desc, id asc')->field("id,title,lv,cover_url")->select();
        foreach ($cates as $cate) {
            $cate['children'] = self::getGroupTreeByType($type, $cate['id'], $lv + 1);
            $result [] = $cate;
        }
        return $result;
    }
